...
void urhajo_mozgas(SDL_Renderer *renderer,int nehezseg,SDL_Window * window, TTF_Font * font,Allapot*kimenet){
    ...
    bool quit = false;
    bool jobbra = true;
    //SDL_TimerID id = SDL_AddTimer(30, idozit, NULL);
    while(!quit){
        SDL_Event event;
        SDL_WaitEvent(&event);
        ...
        switch(event.type){
            ...
            case SDL_USEREVENT:
                ...
                if(j_lenyomott == true && hajo.x_2 <= 440){
                    mozgat_jobbra(renderer,&hajo);
                }
                if(b_lenyomott == true && hajo.x_1 >= 0){
...
//*****************************Lovedek mozgas innentol majd uj modul
...
void lovedek_mozgat(SDL_Renderer*renderer, Lovedek * lov){
    ...
    while(aktualis != NULL){
        ...
        aktualis->y_1 -= 6;
        aktualis->y_2 -= 6;
        //boxRGBA(renderer, aktualis->x_1, aktualis->y_1, aktualis->x_2, aktualis->y_2, 0, 0, 255, 255);
