...
void mentes_fajlba(const char *nev, int pontszam, const char *fajlnev) {
    FILE *fajl = fopen(fajlnev, "a");
    if (fajl == NULL) {
        printf("Hiba: nem sikerült megnyitni a fájlt: %s\n", fajlnev);
        return;
    }

    fprintf(fajl, "%s_%d\n", nev, pontszam); // Szoveg és pontszam mentese
    printf("Sikeresen mentve: %s_%d\n", nev, pontszam); // Hibakeresesi uzenet
    fclose(fajl); // Fajl lezarasa
}
